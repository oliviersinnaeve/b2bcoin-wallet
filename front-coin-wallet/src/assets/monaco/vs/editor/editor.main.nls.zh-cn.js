...
define("vs/editor/editor.main.nls.zh-cn", {
    ...
    "vs/base/common/json": ["符号无效", "数字格式无效", "需要属性名", "需要值", "需要冒号", "需要逗号", "需要右大括号", "需要右括号", "预期的文件结尾"],
    "vs/base/common/keybinding": ["Windows", "控件", "Shift", "Alt", "命令", "Windows", "Ctrl", "Shift", "Alt", "命令", "Windows"],
    ...
    "vs/editor/common/services/modeServiceImpl": ["有助于语言声明。", "语言 ID。", "语言的别名。", "与语言关联的文件扩展名。", "与语言关联的文件名。", "与语言关联的文件名 glob 模式。", "与语言关联的 Mime 类型。", "与语言文件的第一行匹配的正则表达式。", "包含语言配置选项的文件的相对路径。"],
    "vs/editor/common/services/modelServiceImpl": ["[{0}]\n{1}", "[{0}] {1}"],
    "vs/editor/contrib/bracketMatching/common/bracketMatching": ["转到括号"],
    "vs/editor/contrib/caretOperations/common/caretOperations": ["将脱字号向左移", "将脱字号向右移"],
    ...
    "vs/editor/contrib/find/common/findController": ["查找", "查找下一个", "查找上一个", "查找下一个选择", "查找上一个选择", "替换", "将选择添加到下一个查找匹配项", "将选择内容添加到上一查找匹配项", "将上次选择移动到下一个查找匹配项", "将上个选择内容移动到上一查找匹配项", "选择所有找到的查找匹配项", "更改所有匹配项"],
    "vs/editor/contrib/folding/browser/folding": ["展开", "以递归方式展开", "折叠", "以递归方式折叠", "全部折叠", "全部展开", "折叠级别 {0}"],
    "vs/editor/contrib/format/common/formatActions": ["格式化代码", "设置选定内容的格式"],
    "vs/editor/contrib/goToDeclaration/browser/goToDeclaration": [" – {0} 定义", "转到定义", "打开侧边的定义", "查看定义", "转到实现", "速览实现", "单击显示 {0} 个定义。"],
    "vs/editor/contrib/gotoError/browser/gotoError": ["({0}/{1})", "转到下一个错误或警告", "转到上一个错误或警告"],
    ...
    "vs/editor/contrib/linesOperations/common/linesOperations": ["向上复制行", "向下复制行", "向上移动行", "向下移动行", "按升序排列行", "按降序排列行", "裁剪尾随空格", "删除行", "行缩进", "行减少缩进", "在上面插入行", "在下面插入行", "删除左侧所有内容", "Delete All Right", "联接行", "转置游标处的字符", "转换为大写", "转换为小写"],
    "vs/editor/contrib/links/browser/links": ["Cmd + 单击以跟踪链接", "Ctrl + 单击以跟踪链接", "抱歉，无法打开此链接，因为其格式不正确: {0}", "抱歉，无法打开此链接，因为其目标丢失。", "打开链接"],
    ...
    "vs/editor/contrib/quickFix/browser/quickFix": ["显示修补程序({0})", "显示修补程序", "快速修复"],
    "vs/editor/contrib/quickOpen/browser/gotoLine": ["转到行 {0} 和列 {1}", "转到行 {0}", "输入要导航到的介于 1 到 {0} 之间的行号", "输入要导航到的介于 1 到 {0} 之间的列", "Go to line {0}", "输入行号，后跟可选的冒号以及要导航到的列号", "转到行..."],
    "vs/editor/contrib/quickOpen/browser/quickCommand": ["{0}, commands", "输入你想要执行的操作的名称", "命令面板"],
    "vs/editor/contrib/quickOpen/browser/quickOutline": ["{0}, symbols", "输入你想要导航到的标识符的名称", "转到符号...", "符号({0})", "模块({0})", "类({0})", "接口({0})", "方法({0})", "函数({0})", "属性({0})", "变量({0})", "变量({0})", "构造函数({0})", "调用({0})"],
    ...
    "vs/editor/contrib/referenceSearch/browser/referencesWidget": ["解析文件失败。", "{0} 个引用", "{0} 个引用", "无可用预览", "引用", "无结果", "引用"],
    "vs/editor/contrib/rename/browser/rename": ["抱歉，重命名无法执行。", "重命名符号"],
    ...
    "vs/editor/contrib/suggest/browser/suggestController": ["触发建议"],
    "vs/editor/contrib/suggest/browser/suggestWidget": ["阅读更多...{0}", "{0}(建议)具有详细信息", "{0}，建议", "返回", "正在加载...", "无建议。", "{0}，已接受", "{0}(建议)具有详细信息", "{0}，建议"],
    "vs/editor/contrib/toggleTabFocusMode/common/toggleTabFocusMode": ["切换 Tab 键移动焦点"],
    "vs/editor/contrib/zoneWidget/browser/peekViewWidget": ["关闭"],
    "vs/platform/configuration/common/configurationRegistry": ["为一组语言标识符配置要被替代的设置。", "替代设置", "用于配置字符串。", "设置摘要。此标签将在设置文件中用作分隔注释。", "配置属性的描述。", '如果进行设置，"configuration.type" 必须设置为对象', "configuration.title 必须是字符串", "configuration.properties 必须是对象"],
    "vs/platform/extensions/common/extensionsRegistry": ["对于 VS Code 扩展程序，指定该扩展程序与之兼容的 VS Code 版本。不能为 *. 例如: ^0.10.5 表示最低兼容 VS Code 版本 0.10.5。", "VS Code 扩展的发布服务器。", "VS Code 库中使用的扩展的显示名称。", "VS Code 库用于对扩展进行分类的类别。", "VS Code 商城使用的横幅。", "VS Code 商城页标题上的横幅颜色。", "横幅中使用的字体颜色主题。", "由此包表示的 VS Code 扩展的所有贡献。", "在 Marketplace 中设置扩展，将其标记为“预览”。", "VS Code 扩展的激活事件。", "在 Marketplace 的扩展页边栏中显示的徽章数组。", "徽章图像 URL。", "徽章链接。", "徽章说明。", "其他扩展的依赖关系。扩展的标识符始终是 ${publisher}.${name}。例如: vscode.csharp。", "包作为 VS Code 扩展发布前执行的脚本。", "128 x 128 像素图标的路径。"],
    "vs/platform/keybinding/common/abstractKeybindingService": ["以下是其他可用命令:", "已按下({0})。正在等待同时按下第二个键...", "组合键({0}, {1})不是命令。"],
