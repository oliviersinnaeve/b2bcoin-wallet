package com.b2beyond.wallet.monero.utils;

import com.b2beyond.wallet.monero.rpc.model.MoneroAddress;
import com.b2beyond.wallet.monero.rpc.model.MoneroException;
import com.b2beyond.wallet.monero.rpc.model.MoneroIntegratedAddress;
...
public class MoneroUtils {
    ...
    public static void validateStandardAddress(String standardAddress) {
        ...
        if (standardAddress.length() != STANDARD_ADDRESS_LENGTH)
            throw new MoneroException("Standard address is " + standardAddress.length() + " characters but must be " + STANDARD_ADDRESS_LENGTH);
    }
    ...
    public static void validatePaymentId(String paymentId) {
        ...
        if (paymentId.length() != PAYMENT_ID_LENGTH)
            throw new MoneroException("Payment id is " + paymentId.length() + " characters but must be " + PAYMENT_ID_LENGTH);
    }
    ...
    public static void validateIntegratedAddress(String integratedAddress) {
        ...
        if (integratedAddress.length() != INTEGRATED_ADDRESS_LENGTH)
            throw new MoneroException("Integrated address is " + integratedAddress.length() + " characters but must be " + INTEGRATED_ADDRESS_LENGTH);
    }
    ...
    public static void validateMnemonicSeed(String mnemonicSeed) {
        ...
        if (words.length != MNEMONIC_SEED_NUM_WORDS)
            throw new MoneroException("Mnemonic seed must be " + MNEMONIC_SEED_NUM_WORDS + " words but was " + words.length + " words");
    }
    ...
    public static void validateViewKey(String viewKey) {
        ...
        if (viewKey.length() != VIEW_KEY_LENGTH)
            throw new MoneroException("View key must be " + VIEW_KEY_LENGTH + " characters but was " + viewKey.length() + " characters");
    }
...
//        //else if (isValidIntegratedAddress(address)) return SplitIntegratedAddress(address);
